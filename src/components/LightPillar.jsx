...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    // 🔥 canvas ko fully transparent clear karo
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // 🔥 negative remove + clamp
        col = max(col, vec3(0.0));
        col *= uIntensity;

        // 🔥 alpha = brightness -> dark area transparent
        float brightness = clamp(max(col.r, max(col.g, col.b)), 0.0, 1.0);
    ...
    const material = new THREE.ShaderMaterial({
      ...
      depthWrite: false,
      depthTest: false,
      blending: THREE.AdditiveBlending // 🔥 glow only, no darkening
    });
  ...
  if (!webGLSupported) {
    return (
      <div
        ...
        style={{ mixBlendMode, background: 'transparent' }} // ❌ black hata diya
      >
        WebGL not supported
      </div>
    );
